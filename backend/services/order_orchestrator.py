...
def generate_pdf_document(order_no, client_name, payload_fiscal, payload_log):
    ...
    for i, item in enumerate(payload_fiscal):
        ...
        pdf.cell(30, 5, str(float(item['Cantitate (U.M.)'].split(' ')[0])), 'L,T,R', 1, 'C')
        
        pdf.cell(10, 4, " ", 'L,B,R', 0, 'C')
        pdf.set_text_color(100, 100, 100)
        pdf.cell(130, 4, "Cod NC: 48236990", 'L,B,R', 0, 'L')
    ...
    pdf.cell(0, 5, f"Nr. {order_no} / Data: {data_azi}", align='C', ln=1)
    pdf.ln(10)
    
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(10, 8, "Nr", 1, 0, 'C')
    pdf.cell(30, 8, "Cod Raft", 1, 0, 'C')
    pdf.cell(110, 8, "Denumire / Instructiune Stivuitorist", 1, 0, 'C')
    pdf.cell(20, 8, "Cant", 1, 0, 'C')
    pdf.cell(20, 8, "U/M", 1, 1, 'C')
    ...
    for i, item in enumerate(payload_log):
        ...
        pdf.cell(30, 8, str(item.get('Cod Gestiune', '-')), 1, 0, 'C')
        pdf.set_font("Arial", '', 9)    
        pdf.cell(110, 8, clean_text(item['Denumire'][:85]), 1, 0)
        pdf.cell(20, 8, str(item['Cant']), 1, 0, 'C')
        if is_palet: 
            pdf.set_fill_color(220, 220, 220)
            pdf.cell(20, 8, item['UM'], 1, 1, 'C', fill=True)
        else: 
            pdf.cell(20, 8, item['UM'], 1, 1, 'C')
...
def render_lansare_module():
    ...
    with tab1:
        ...
        if len(comenzi_asteapta_acte) > 0:
            st.error(f"🚨 ACȚIUNE NECESARĂ: {len(comenzi_asteapta_acte)} comandă(i) au fost încărcate! Treci la Rampă.")
        ...
        if not st.session_state.mod_previzualizare:
            ...
            col_s3.metric("🔄 WMS", f"{p_data['conversion']} cut/pal")
            col_s4.metric("⚖️ Fiscal", f"{p_data['conversie_baza']} {p_data['um_baza']}/cut")
        ...
        # PREVIZUALIZARE DIMO-NEXUS
            ...
            def executa_lansare():
                for item in st.session_state.schita_comanda:
                    p = item['Produs']
                    P = st.session_state.db[p]['conversion']
                    C = item['Cutii']
                    pal = item['Paleti']
                    s_ramas = get_total_boxes(p) - ((pal * P) + C)
                    st.session_state.db[p]['stock_pal'] = s_ramas // P
                    st.session_state.db[p]['stock_box'] = s_ramas % P
