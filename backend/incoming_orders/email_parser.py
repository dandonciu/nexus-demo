...
def render_email_parser_module():
    ...
    st.info("🤖 Botul NEXUS monitorizează adresa de email (ex: nova_facturi@novasafe.ro) pentru a prelua și decoda fișierele cu facturi - Excel sau PDF - de la clienți și a le trimite în format normalizat către SmartBill.")
    st.title("📨 Documentele se trimit spre arhivare în Vault.")
    st.divider()
