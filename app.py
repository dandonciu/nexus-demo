...
def generate_pdf_document(order_no, client_name, payload_fiscal, payload_log):
    ...
    text_decl = f"Noi, {f_data['Nume']}, cu sediul in {clean_text(f_data['Adresa'])}, asiguram, garantam si declaram pe propria raspundere, ca produsele enumerate mai jos, livrate cu Avizul Nr. {order_no}/{data_azi}, nu pun in pericol viata, sanatatea si sunt conforme."
...
st.markdown("""
    <div style="background: linear-gradient(90deg, #003366 0%, #004080 100%); padding: 20px 30px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; color: white; margin-bottom: 25px;">
        ...
        <div style="text-align: right;"><p style="margin: 0; padding: 0; font-weight: bold; font-size: 1.3rem;">NOVA SAFE SRL</p></div>
    </div>
        ...
        if not st.session_state.mod_previzualizare:
            ...
            if len(st.session_state.schita_comanda) > 0:
                ...
                for idx, item in enumerate(st.session_state.schita_comanda):
                    st.markdown(f"- **{item['Produs']}** ({item['Paleti']} Paleti, {item['Cutii']} Cutii)")
                
                if st.button("👁️ Aplică Regula Dimo-NEXUS & Finalizează", type="primary"):
                    st.session_state.client_temporar_comandat = client_ales
                    st.session_state.mod_previzualizare = True
                    st.rerun()

        # --- ECRAN B: PREVIZUALIZARE & SCINDARE DIMO-NEXUS ---
        else:
            ...
            for item in st.session_state.schita_comanda:
                ...
                # --- WMS LOGIC (Dimo-NEXUS) Adaptat pentru FPDF ---
                if C > 0:
                    if (P - C) < C and L < C:
...
# ==========================================
    ...
    df_filtrat = df_toate[df_toate['Client'] == analiza_client] if not df_toate.empty else pd.DataFrame()
    
    if df_filtrat.empty:
        st.warning(f"Nu există date de livrare pentru {analiza_produs} către {analiza_client}.")
    else:
        fig = px.bar(df_filtrat, x='Data', y='Volum_Paleti', color='Status_Plata', text='Volum_Paleti', title="Volum Livrări")
